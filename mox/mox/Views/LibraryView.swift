...
struct LibraryView: View {
    ...
    private var footer: some View {
        HStack {
            Label(model.modelPath, systemImage: "folder")
            Spacer()
            Text(String(format: "%d개 모델 · %.1f GB 사용 중", model.installedModels.count, model.usedDiskGB))
        }
        .font(.system(size: 12))
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .background(isActive ? Color.accentColor.opacity(0.15) : .clear, in: Capsule())
                .overlay(Capsule().strokeBorder(isActive ? Color.accentColor : Color.secondary.opacity(0.3), lineWidth: 0.5))
        }
        .buttonStyle(.plain)
    }
}
